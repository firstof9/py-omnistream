"""Websocket class for py-omnistream."""
...
class OmniStreamWebsocket:
    ...
    async def running(self):
        ...
        await OmniStreamWebsocket.state.fset(self, STATE_STARTING)
        ...
        try:
            async with self.session.ws_connect(
                ...
                await OmniStreamWebsocket.state.fset(self, STATE_CONNECTED)
                self.failed_attempts = 0
            ...
            await OmniStreamWebsocket.state.fset(self, STATE_STOPPED)
            ...
            if self.failed_attempts >= MAX_FAILED_ATTEMPTS:
                ...
                await OmniStreamWebsocket.state.fset(self, STATE_DISCONNECTED)
    ...
    async def ws_msg(self,request: dict):
        """Send message via websocket."""
        request = json.dumps(request)
        _LOGGER.debug("Sending message: %s", request)
        await self._ws_client.send_str(request)
